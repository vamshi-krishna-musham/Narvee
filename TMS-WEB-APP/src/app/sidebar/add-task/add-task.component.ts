...
export class AddTaskComponent {
  ...
  goBack(): void {
    this.router.navigate(['/projects']); // or wherever you want to go back to
  }
  ...
  ngOnInit() {
    ...
    this.canAddProject = privileges.includes('CREATE_TASK');
    this.canEditProject = privileges.includes('EDIT_TASK');
    this.canDeleteProject = privileges.includes('DELETE_TASK');
    this.canSeeProjectActions = privileges.includes('TASK_ACTION_VISIBLE');
    this.route.queryParams.subscribe(params => {
      ...
      console.log(this.projectId, 'taskpageprojectid');
      console.log(this.targetDate, 'this.targetDatengononit');
          console.log(this.startDate, 'this.startDatengonint');
 ...
 addtaskview() {
  if (this.status === 'Completed') {
    const snackBarData: ISnackBarData = {
      message: 'Cannot add a task. Project status is completed.',
      duration: 3000,
      verticalPosition: 'top',
      horizontalPosition: 'center',
  ...
  const dialogRef = this.dialog.open(AddTaskviewComponent, {
    width: '100%',
    maxWidth: '700px',
    disableClose: true,
    data: {
  ...
  dialogRef.afterClosed().subscribe((result) => {
    if (result === true) {
      this.getprojectdetails(this.currentPageIndex + 1);
    }
  });
}
  ...
  edittaskview(task: any) {
    const dialogRef = this.dialog.open(AddTaskviewComponent, {
      ...
      data: {
        ...
        taskId: task.taskid  // ✅ Send selected taskid
      }
    });

    dialogRef.afterClosed().subscribe((result) => {
      if (result === true) {
        this.getprojectdetails(this.currentPageIndex + 1); // refresh table data
      }
    });
  }
  ...
  getprojectdetails(pagIdx = 1) {
    ...
    return this.projectServ.getprojectTasks(pagObj)
      .pipe(takeUntil(this.destroyed$))
      .subscribe({
        next: (response: any) => {
          this.totalItems = response.data.tasks.totalElements || 0;
          this.dataSource.data = response.data.tasks.content.map((x: any, i: number) => ({
            ...x,
...
onStatusChange(taskId: any, newStatus: any, element: any): void {
  const updatedby = localStorage.getItem('profileId');
  if (!updatedby) {
  ...
  const oldStatus = element.status; // backup old status
  this.editingStatusId = null;

  this.projectServ.updatetaskstatus(taskId, newStatus, updatedby).subscribe({
    next: (res: any) => {
  ...
  handlePageEvent(event: PageEvent) {
    ...
    this.getprojectdetails(this.currentPageIndex + 1); // 1-based index for API
  }
  ...
  addsubtask(taskid: number, ticketid: number,targetdate:any,startdate:any ,status:any ,taskName :any) {
    this.router.navigate(['/add-subtask'], {
      queryParams: {
        ...
        pid: this.pid,
        status:status,
        projectStatus: this.status ,     // ✅ project status from project route param
       taskName:taskName,
    ...
    dialogRef.afterClosed().subscribe({
      next: () => {
        if (dialogRef.componentInstance.allowAction) {
          this.projectServ.DeleteTaskDetails(task.taskid).pipe(takeUntil(this.destroyed$)).subscribe({
            next: (response: any) => {
              if (response.status === 'success') {
                this.getprojectdetails();
                this.dataTobeSentToSnackBarService.message = response.message || 'Task Deleted Successfully';
                this.dataTobeSentToSnackBarService.panelClass = ['custom-snack-success'];
              } else {
                this.dataTobeSentToSnackBarService.message = response.message || 'Task Deletion Failed';
                this.dataTobeSentToSnackBarService.panelClass = ['custom-snack-failure'];
              }
              this.snackBarServ.openSnackBarFromComponent(this.dataTobeSentToSnackBarService);
            },
            error: (err: any) => {
              this.dataTobeSentToSnackBarService.message = err.message || 'Task Deletion Failed';
              this.dataTobeSentToSnackBarService.panelClass = ['custom-snack-failure'];
              this.snackBarServ.openSnackBarFromComponent(this.dataTobeSentToSnackBarService);
            },
          });
  ...
  submitComment(): void {
    ...
    this.projectServ.commentAddTask(payload).subscribe({
      next: () => {
        this.snackBarServ.openSnackBarFromComponent({
  ...
  getcommentTask(): void {
    this.projectServ.getcommentTask(this.selectedTaskId).subscribe({
      next: (res: any) => {
...
openTemplateDialog(templateRef: TemplateRef<any>, row?: any) {
  ...
  if (row?.taskid) {
    this.selectedTaskId = row.taskid;
    this.getuploadfiles(); // Fetch latest uploaded files
  }
...
submitFiles() {
  ...
  this.projectServ.saveuploadfile(formData).subscribe({
    next: () => {
      ...
      this.getprojectdetails(); // ✅ call your method here
...
getuploadfiles() {
  ...
  this.projectServ.getuploadfile(this.selectedTaskId).subscribe((res: any) => {
...
// updateReportFile(): void {
//   if (this.selectedFilesReport.length === 0) {
//     this.snackBarServ.openSnackBarFromComponent({
//       message: 'No new files selected for update.',
//       duration: 2500,
//       verticalPosition: 'top',
//       horizontalPosition: 'center',
//       panelClass: ['custom-snack-failure']
...
//   const formData = new FormData();
//   this.selectedFilesReport.forEach(file => {
//     formData.append('file', file);
//   });

//   const fileId = this.existingFilesReport[0]?.id;
//   if (!fileId) {
//     this.snackBarServ.openSnackBarFromComponent({
//       message: 'No existing file ID found to update.',
//       duration: 2500,
//       verticalPosition: 'top',
//       horizontalPosition: 'center',
//       panelClass: ['custom-snack-failure']
...
//   this.projectServ.updateuploadfile(fileId, formData).subscribe({
//     next: (res: any) => {
//       const isSuccess = res?.status === 'success';
//       const message = res?.message || (isSuccess ? 'File updated successfully!' : 'Update failed.');
