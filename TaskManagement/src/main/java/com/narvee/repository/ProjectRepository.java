...
public interface ProjectRepository extends JpaRepository<TmsProject, Long> {
	...
	@Query(value = "SELECT  p.pid , p.projectname , p.projectdescription , p.addedby , p.status , p.updatedby , p.projectid  , p.createddate ,p.updateddate, p.department FROM tms_project p ", nativeQuery = true)
	public Page<ProjectDTO> findAllProjects(Pageable pageable, @Param("keyword") String keyword);
    
	@Query(value = "SELECT  p.pid , p.projectname , p.projectdescription , p.addedby , p.status , p.updatedby , p.projectid  , p.createddate ,p.updateddate  FROM tms_project p  where p.addedby = :addedby", nativeQuery = true)
	public Page<ProjectDTO> findAllProjects(Pageable pageable, Long addedby);

	
	
	@Query(value = "SELECT  p.pid , p.projectname , p.projectdescription , p.addedby , p.status , p.updatedby , p.projectid , p.createddate ,p.updateddate, p.department  FROM tms_project p where p.pid= :pid ", nativeQuery = true)
  ...
  /////-----------------Replicated  methods for tms project --------------
	...
	@Query(value = "SELECT  p.pid , p.projectname , p.projectdescription , DATE(p.start_date) AS startDate , DATE(p.target_date) AS targetDate ,p.addedby ,  concat(tu.first_name,' ',tu.middle_name,' ',tu.last_name) AS addedByFullname, p.status , p.updatedby , p.projectid  , p.createddate ,p.updateddate"
	...
	@Query(value = "SELECT p.pid ,p.projectname , p.projectdescription , DATE(p.start_date) AS startDate , DATE(p.target_date) AS targetDate, p.addedby , concat(tu.first_name,' ',tu.middle_name,' ',tu.last_name) AS addedByFullname , p.status , p.updatedby , p.projectid , p.createddate ,p.updateddate, p.department "
